...
// 11 - List Flatten
// Метод flatten превращает список списков List[List[T]] в один плоский список List[T].
...
// 12 - flatMap
// flatMap — это комбинация двух операций: сначала map, затем flatten. То есть мы трансформируем каждый
// элемент списка в другой список (или коллекцию) и объединяем результат в один «плоский» список.
...
// 13 — foldLeft
// foldLeft — это метод для последовательной обработки элементов коллекции с аккумулятором (начальным значением).
...
// for-comprehension в Scala — это удобный способ работать с коллекциями (или другими типами вроде Option, Future)
// в функциональном стиле, позволяя итерировать, фильтровать и трансформировать элементы.
